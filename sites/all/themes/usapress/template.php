<?php
/**
 * @file
 * Contains the theme's functions to manipulate Drupal's default markup.
 *
 * Complete documentation for this file is available online.
 * @see https://drupal.org/node/1728096
 */

/**
 * Implements hook_theme().
 */
function usapress_theme() {
  return array(
    'usapress_color_css' => array(
      'variables' => array(
        'tune_in' => NULL,
      ),
      'template' => 'templates/color.css'
    ),
  );
}

/**
 * Implements template_preprocess_html().
 */
function usapress_preprocess_html(&$variables) {
  drupal_add_css(theme('usapress_color_css'), 'inline');
}

/**
 * Implements template_preprocess_usapress_color_css().
 */
function usapress_preprocess_usa_color_css(&$variables) {
  $colors = &drupal_static('usapress_color_css', array());
  foreach ($colors as $name => $color) {
    if (empty($variables[$name])) {
      $variables[$name] = $color;
    }
  }
  $variables += $colors;
}

/**
 * Implements theme_preprocess_panels_pane().
 */
function usapress_preprocess_panels_pane(&$variables, $hook) {
  if($variables['pane']->panel == 'header_image' && $variables['pane']->subtype == 'node:url') {
    $variables['theme_hook_suggestions'][] = 'panels_pane__project_node';
  }
}

/**
 * Implements theme_preprocess_field().
 */
function usapress_preprocess_field(&$variables) {
  $colors = &drupal_static('usapress_color_css', array());
  $field = &$variables['element'];
  if ($field['#field_name'] == 'field_press_kit_date_text' || $field['#field_name'] == 'field_press_kit_date_date') {
    $entity_type = $field['#entity_type'];
    $entity = $field['#object'];
    if ($color = field_get_items($entity_type, $entity, 'field_press_kit_date_color')) {
      $color = reset($color);
      if ($color) {
        $colors['tune_in'] = $color['rgb'];
      }
    }
  }
}
