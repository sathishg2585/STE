...
Drupal.behaviors.my_custom_behavior = {
  ...
  Drupal.behaviors.adaptive_body_class = {
    attach: function(context, settings) {
      var bodySingle = $('.sy-fy-press-single-homepage'),
          bodyTablet = 'tablet',
          tabletWidth = 800;
  ...
  * */
    ...
    attach: function(context, settings) {
      ...
      if (sliderWrapper.length !== 0) {
        var slidesContainer = sliderWrapper.find('.slides'),
            slide = slidesContainer.find('.field-name-field-syfy-photos-photo'),
            pageTitle = $('.pane-node-title'),
            photoContainer = $('.pane-node-field-syfy-photos'),
            fakeDownloadClass = 'fake-photos-download-link',
            realDonwloadClass = 'field-name-field-syfy-photos-download-link',
  ...
  Drupal.behaviors.links_item_height = {
    attach: function(context, settings) {
      var linkWrapper = $('.view-syfy-links-view', context),
          tabletWidth = 1024;
      ...
      function setItemHeight (option) {
        var linkItemArr = linkWrapper.find('.syfy-links-row'),
  ...
  * Syfy logo position
  * */
  Drupal.behaviors.syfy_logo_position = {
    attach: function(context, settings) {
      var logoPosition = $('.pane-node-field-press-kit-logo-position', context);
      // Check if slider exists
      if (logoPosition.length !== 0) {
        $('.syfy-homepage-single-project-wrapper').addClass('syfy-logo-block-midway');
        if ($(".pane-node-field-press-kit-date-text").length === 0 && $(".pane-node-field-press-kit-date-date").length === 0) {
          $(".pane-syfy-press-logo").addClass("syfy-press-logo-center");
        }
      }
    }
  }
